...
var spawnManager = {
    ...
    run: function(spawn) {
        ...
        //===== DEFENDERS
        let hostiles = _.filter(spawn.room.find(FIND_HOSTILE_CREEPS), c => (c.getActiveBodyparts(ATTACK) > 0 || c.getActiveBodyparts(RANGED_ATTACK) > 0));
	...
	spawnCreep: function(spawn, body, role, mem = {}) {
	    let memory = _.assign(mem, { role: role })
	    var result = spawn.spawnCreep(body, role + '_' + Game.time, {memory: memory});
